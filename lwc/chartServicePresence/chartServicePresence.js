...
import getChartData from '@salesforce/apex/ChartServicePresenceController.getChartData';
import getChannels from '@salesforce/apex/ChartServicePresenceController.getChannels';

export default class ChartServicePresence extends LightningElement {
    ...
    initializeWorkItemsChart = (data) => {
        try {
            ...
            if (!canvas) {
                throw new Error('No se encontró el elemento <canvas.canvas-chart-cnt> en el DOM.');
            }
...
//                                callback: this.ticksCallback,
                    ...
                    plugins: {
                        ...
                        title: {
                            display: false,
                            text: 'Presencia por Usuario por Hora',
                            padding: {
                                bottom: 20
                              }
                        },
                        tooltip: {
                            ...
                            callbacks: {
                                label: function (tooltipItem) {
                                    ...
                                    const isEnd = index === dataPoints.length - 1;
                        
                                    if (isStart || isEnd) {
                                        const startTime = formatMinutesToTime(dataPoints[0].x);
                                        const endTime = formatMinutesToTime(dataPoints[dataPoints.length - 1].x);
                                        const freeTime = dataset.recordId || 'N/A';
                        
                                        return `Estado: ${label}, Hora inicio: ${startTime} - Hora Fin: ${endTime} - Tiempo libre: ${freeTime}`;
                                    } else {
                                        if (dataset.recordId != null) {
                                            return `Hora: ${formatMinutesToTime(value.x)}, Estado: ${label}, Tiempo libre: ${dataset.recordId}`;
                                        } else {
                                            return `Hora: ${formatMinutesToTime(value.x)}, Estado: ${label}`;
                                        }
                                    }
                                }
                            }
